...
function applyDragForce() {
	...
	if (Math.abs(globals.ballArr[store.heldBalls[0]].velocityX) > 0 && Math.abs(globals.ballArr[store.heldBalls[0]].velocityY) > 0) {
		// has stopped moving, remove from array
		...
		//globals.ballArr[store.heldBalls[0]].hasInertia = false;
	}



	if ( !globals.ballArr[store.heldBalls[0]].hasInertia ) {
		//return;
	}
...
//if a circle stops moving, remove it from the array
...
const updateInertia = function() {
	...
	if (globals.ballArr[store.heldBalls[0]].hasInertia === false) {
		return;
	}

		console.log(Math.floor(Math.abs(globals.ballArr[store.heldBalls[0]].velocityX)))
	...
	//Math.floor(Math.abs(globals.ballArr[store.heldBalls[0]].velocityX)) === 0
